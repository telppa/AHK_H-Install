...
BinFiles := []
BinNames := ""
ScanDir := ["U64", "U32", "A32"]	; 限制搜索范围在 U64 U32 A32 这3个目录中
for k, DirName in ScanDir
{
	Loop, %A_ScriptDir%\..\%DirName%\*.bin,0,1
	{
	...
	Loop, %A_ScriptDir%\..\%DirName%\*.exe,0,1
	{
		SplitPath,A_LoopFileFullPath,,d,, n
		FileGetVersion, v, %A_LoopFileFullPath%
	...
	Loop, %A_ScriptDir%\..\%DirName%\*.dll,0,1
	{
		SplitPath, A_LoopFileFullPath,,d,, n
		FileGetVersion, v, %A_LoopFileFullPath%
		If !InStr(FileGetInfo(A_LoopFileFullPath,"FileDescription"),"AutoHotkey")
